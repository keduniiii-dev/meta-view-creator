...
const homeUrl = crmPath();

const items = [
  { title: "Home", url: homeUrl },
  { title: "Dashboard", url: crmPath("/dashboard") },
  { title: "Leads", url: crmPath("/leads") },
  { title: "Pipeline", url: crmPath("/pipeline") },
  { title: "Outreach", url: crmPath("/outreach") },
  { title: "EMEA", url: crmPath("/emea") },
  { title: "Americas", url: crmPath("/americas") },
  { title: "Analytics", url: crmPath("/analytics") },
];
...
const CrmLayout = () => {
  ...
  const handleLogout = async () => {
    await logout();
    navigate(crmPath("/login"), { replace: true });
  };
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-40 border-b border-border bg-background/95 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-7xl items-center gap-2 px-4 sm:px-6">
          <NavLink to={homeUrl} className="flex shrink-0 items-center gap-2 text-sm font-semibold text-foreground">
          ...
          <nav className="hidden flex-1 items-center justify-center gap-1 xl:flex" aria-label="CRM navigation">
            {items.map((item) => item.url ? (
              <NavLink key={item.url} end={item.url === homeUrl} to={item.url} className={({ isActive }) => `rounded-md px-3 py-2 text-xs transition-colors ${isActive ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}>{item.title}</NavLink>
            ) : <span key={item.title} className="rounded-md px-3 py-2 text-xs text-muted-foreground">{item.title}</span>)}
          </nav>
          <div className="ml-auto flex items-center gap-2">
            <Button size="sm" className="hidden h-11 text-xs sm:h-8 sm:inline-flex" onClick={() => navigate(crmPath("/capture"))}>Get Started</Button>
                ...
                <nav className="flex flex-col gap-1 p-4" aria-label="CRM navigation">
                  {items.map((item) => item.url ? (
                    <SheetClose asChild key={item.url}>
                      <NavLink end={item.url === homeUrl} to={item.url} className={({ isActive }) => `flex min-h-11 items-center rounded-md px-3 py-2 text-sm transition-colors ${isActive ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}>{item.title}</NavLink>
                    </SheetClose>
                  ...
                  <div className="mt-2 border-t border-border pt-2">
                    <Button variant="outline" className="flex min-h-11 w-full items-center justify-center text-sm" onClick={() => navigate(crmPath("/capture"))}>Get Started</Button>
                  </div>
                </nav>
              </SheetContent>
            </Sheet>
