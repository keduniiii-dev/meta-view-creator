...
const Analytics = () => (
  <div className="p-6 lg:p-8">
    ...
    <p className="text-muted-foreground mb-8">Performance metrics, engagement funnels, and campaign ROI</p>
      ...
      {kpis.map((kpi, i) => (
        <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            {kpi.trend === "up" ? <ArrowUpRight className="w-4 h-4 text-success" /> : <ArrowDownRight className="w-4 h-4 text-destructive" />}
          </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
      <div className="gradient-card rounded-xl border border-border p-6 shadow-card">
        <h3 className="text-lg font-semibold text-foreground mb-4">Lead Acquisition Trend</h3>
        ...
        {conversionFunnel.map((step, i) => (
          <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }} className="flex items-center gap-4">
            ...
            <div className="flex-1 h-8 bg-secondary rounded-md overflow-hidden relative">
              <div className="h-full rounded-md gradient-primary transition-all duration-700" style={{ width: `${step.pct}%` }} />
